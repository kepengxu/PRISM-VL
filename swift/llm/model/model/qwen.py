...
def _forward_qwen3_vl_or_qwen3_omni(
    ...
    if pixel_values is None and pixel_values_videos is None:  # plain-text
        ...
        image_embeds, deepstack_visual_embeds = self.visual(pixel_values, grid_thw=media_inputs['image_grid_thw'], metadata=metadata)
        inputs_embeds = inputs_embeds + image_embeds.mean().to(device=inputs_embeds.device) * 0.
        visual_pos_masks = None
        # git commit -m '准备元数据自适应VLM'
    else:
        ...
        mixed_embeds, deepstack_visual_embeds = self.visual(pixel_values_mixed, grid_thw=grid_thw, metadata=metadata)
        if pixel_values is None:
            image_embeds = None
...
def _qwen3_vl_camera_metadata_enabled(model) -> bool:
    ...
    return bool(getattr(vision_config, 'camera_metadata_residual_enabled', False))


def _qwen3_vl_has_camera_metadata(kwargs) -> bool:
    for key in ('iso', 'exp_time', 'aperture'):
        value = kwargs.get(key)
        if value is None:
            continue
        if isinstance(value, (list, tuple)) and all(v is None for v in value):
            continue
        return True
    return False
...
def _compat_qwen3_vl_mixed_data(model, processor, is_moe: bool = False):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        use_camera_metadata_path = _qwen3_vl_camera_metadata_enabled(self) and _qwen3_vl_has_camera_metadata(kwargs)
        if not self.training and not is_deepspeed_enabled() and not use_camera_metadata_path:
        ...
        # isos = torch.tensor(kwargs['iso']).to(input_ids.device) if 'iso' in kwargs else None
        # exp_times = torch.tensor(kwargs['exp_time']).to(input_ids.device) if 'exp_time' in kwargs else None
        # apertures = torch.tensor(kwargs['aperture']).to(input_ids.device) if 'aperture' in kwargs else None
        
        # isos = torch.log2(torch.tensor(kwargs['iso'])/100)/10 if 'iso' in kwargs else None
        # exp_times = torch.tensor(kwargs['exp_time'])*30 if 'exp_time' in kwargs else None
        # apertures = 1.0/torch.tensor(kwargs['aperture']) if 'aperture' in kwargs else None
        
        metadata = {
            'iso': kwargs['iso'] if 'iso' in kwargs else None,
            'exp_time': kwargs['exp_time'] if 'exp_time' in kwargs else None,
            'aperture': kwargs['aperture'] if 'aperture' in kwargs else None,
        }
...
def get_model_tokenizer_qwen3_vl(model_dir, *args, **kwargs):
    ...
    model_kwargs = args[1] if len(args) >= 2 and isinstance(args[1], dict) else None
    model_config = kwargs.get('model_config')
    if model_config is None:
        model_config = AutoConfig.from_pretrained(model_dir, trust_remote_code=True)
    vision_config = getattr(model_config, 'vision_config', None)
    custom_vision_keys = (
        'camera_metadata_residual_enabled',
        'camera_metadata_residual_num_layers',
        'camera_metadata_require_all',
        'camera_metadata_norm_scheme',
    )
    if vision_config is not None and model_kwargs is not None:
        for key in custom_vision_keys:
            if key in model_kwargs:
                setattr(vision_config, key, model_kwargs.pop(key))
    kwargs['model_config'] = model_config
