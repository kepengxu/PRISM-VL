...
def load_image(image: Union[str, bytes, Image.Image]) -> Image.Image:
    ...
    if isinstance(image, BytesIO):
        ...
        # print('cv2 imdecode from buffer')
        image = cv2.imdecode(np.frombuffer(image.getbuffer(), np.uint8), cv2.IMREAD_UNCHANGED)
        if image.dtype == np.uint8:
            logger.debug('Decoded uint8 image from in-memory buffer.')
        if image.ndim == 2:
            # grayscale to BGR
        ...
        exception = f'Unsupported image type: {type(image)}'
