...
class Template(ProcessorMixin):
    ...
    def init_processor(self, processor: Processor) -> None:
        ...
        if self.model_meta.is_multimodal:
            ...
            if hasattr(processor, 'image_processor'):
                ...
                logger.info(f'image_processor_type: {image_processor_type}')
    ...
    def _preprocess_inputs(
        ...
        if os.environ.get('SWIFT_DEBUG_METADATA', '0') == '1':
            debug_count = getattr(self, '_debug_preprocess_inputs_count', 0)
            if debug_count < 20:
                extra_keys = sorted(inputs.extra_kwargs.keys()) if isinstance(inputs.extra_kwargs, dict) else []
                logger.info(
                    f'[SWIFT_DEBUG_METADATA][template_pre] idx={debug_count} '
                    f'image_count={len(inputs.images or [])} extra_keys={extra_keys}')
                setattr(self, '_debug_preprocess_inputs_count', debug_count + 1)
    ...
    def _encode_truncated(self, inputs: StdTemplateInputs):
        ...
        # print(self.truncation_strategy)
        if self.max_length is not None and length > self.max_length:
            logger.warning('Row length exceeds max_length; applying truncation strategy.')
