...
class LazyLLMDataset(Dataset):
    ...
    def __init__(self,
        ...
        logger.debug(f"dataset class: {dataset.__class__.__name__}")
    ...
    def getimage_sizes(self):
        from PIL import Image
        self.image_sizes = []
        for i in range(len(self.dataset)):
            data = self.dataset[i]
            if 'images' in data.keys() and (data['images'] is not None) and isinstance(data['images'], list) and len(data['images']) > 0:
                # print(data['images'])
                if 'path' in data['images'][0].keys():
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for i in range(self.n_try_fetch):
            ...
            data = self.dataset[i]
            try:
                data = self.encode_func(data, return_length=True)
                return data
            except Exception:
...
class EncodePreprocessor(RowPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if os.environ.get('SWIFT_DEBUG_METADATA', '0') == '1' and self._debug_count < 20:
            row_keys = sorted(row.keys())
            meta_keys = [k for k in ['iso', 'exp_time', 'aperture'] if k in row]
            logger.info(
                f'[SWIFT_DEBUG_METADATA][encode_pre] idx={self._debug_count} '
                f'row_keys={row_keys} meta_keys={meta_keys}')
        encoded = self.template.encode(row, return_length=True)
        if os.environ.get('SWIFT_DEBUG_METADATA', '0') == '1' and self._debug_count < 20:
            encoded_keys = sorted(encoded.keys()) if isinstance(encoded, dict) else type(encoded).__name__
            logger.info(
                f'[SWIFT_DEBUG_METADATA][encode_post] idx={self._debug_count} '
                f'encoded_keys={encoded_keys}')
            self._debug_count += 1
        return encoded
