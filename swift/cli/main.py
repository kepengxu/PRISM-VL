...
def prepare_config_args(argv):
    for i in range(len(argv)):
        if argv[i] == '--config':
            ...
            # Save config path to env before popping
            os.environ['SWIFT_CONFIG_PATH'] = os.path.abspath(argv[i + 1])
...
def cli_main(route_mapping: Optional[Dict[str, str]] = None, is_megatron: bool = False) -> None:
    ...
    argv = sys.argv[1:]
    if not argv:
        logger.error(f'Usage: swift <command>. Available commands: {", ".join(sorted(route_mapping))}')
        sys.exit(1)
    _compat_web_ui(argv)
    method_name = argv[0].replace('_', '-')
    if method_name not in route_mapping:
        logger.error(f'Unknown command `{method_name}`. Available commands: {", ".join(sorted(route_mapping))}')
        sys.exit(2)
    argv = argv[1:]
    ...
    logger.info(f"run sh: `{' '.join(args)}`")
