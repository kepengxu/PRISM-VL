...
class Seq2SeqTrainer(SwiftMixin, DataLoaderMixin, HfSeq2SeqTrainer):
    ...
    def training_step(self, model, inputs, *args, **kwargs):
        if os.environ.get('SWIFT_DEBUG_METADATA', '0') == '1':
            debug_count = getattr(self, '_debug_training_step_count', 0)
            if debug_count < 20:
                keys = sorted(inputs.keys())
                meta_keys = [k for k in ['iso', 'exp_time', 'aperture', '_extra_kwargs'] if k in inputs]
                extra_keys = []
                if '_extra_kwargs' in inputs:
                    extra = inputs['_extra_kwargs']
                    if isinstance(extra, dict):
                        extra_keys = sorted(extra.keys())
                    elif isinstance(extra, list) and extra and isinstance(extra[0], dict):
                        extra_keys = sorted(extra[0].keys())
                logger.info(
                    f'[SWIFT_DEBUG_METADATA][training_step] step={debug_count} '
                    f'keys={keys} meta_keys={meta_keys} extra_keys={extra_keys}')
                setattr(self, '_debug_training_step_count', debug_count + 1)
        with self.template.forward_context(self.model, inputs):
