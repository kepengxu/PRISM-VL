...
def parse_args(class_type: Type[_T], argv: Optional[List[str]] = None) -> Tuple[_T, List[str]]:
    ...
    # Retrieve the original --config yaml path from env (set by prepare_config_args in cli/main.py)
    _config_path = os.environ.get('SWIFT_CONFIG_PATH')
    if _config_path:
        args._config_path = _config_path
