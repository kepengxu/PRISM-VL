...
def fetch_image(ele: Dict[str, Union[str, Image.Image]], image_patch_size: int = 14) -> Image.Image:
    ...
    if "resized_height" in ele and "resized_width" in ele:
        warnings.warn('Using explicit resized_height/resized_width from input.', stacklevel=2)
...
def fetch_image_cv2(ele: Dict[str, Union[str, Image.Image]], image_patch_size: int = 14) -> Image.Image:
    if "image" in ele:
        image = ele["image"]
    else:
        image = ele["image_url"]
...
# def fetch_image_cv2(
#     ele: Dict[str, Union[str, Image.Image, np.ndarray]],
...
#             raise ValueError(f"cv2 image must be HxWx3, got shape {image.shape}.")
#         if image.dtype == np.uint16:
#             image_8bit = (image >> 8).astype(np.uint8)
#         elif image.dtype == np.uint8:
#             image_8bit = image
#         else:
#             raise ValueError(f"cv2 image must be uint16 or uint8, got {image.dtype}.")
#         image_rgb = image_8bit[..., ::-1].copy()
#         image_obj = Image.fromarray(image_rgb, mode="RGB")
#         ele = dict(ele)
#         ele["image"] = image_obj
